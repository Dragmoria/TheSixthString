...
<form class="ms-auto me-auto mb-5 card p-1 bg-card-custom" style="width: 1200px;"  method="POST" action="/Register" onsubmit="return validatePasswords()">
        ...
        <div class="row">
            <?php foreach ($fields as $label => $field): ?>
                <div class="col-3 ms-4 mb-3">
                    <input type="form-check-text" class="form-control bg-beige-color" id="<?= $field['name'] ?>" name="<?= $field['name'] ?>" placeholder="<?php echo $label; ?>" <?php echo ($field['importance'] === 'required') ? 'required' : ''; ?>>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
    <div class="container">
        ...
        <div class="col-3 ms-4 mb-3">
            ...
            <div class="col-3 ms-4 mb-3">
                <input type="date" class="form-control bg-beige-color" id="birthdate" name="birthdate" min="1900-01-01" max="2050-12-31" / required>
            </div>
        </div>
    </div>
    <div class="container">
                ...
                <div class="col-3 ms-4 mb-3">
                    <?php if ($name === 'password' || $name === 'repeatPassword'): ?>
                        <div class="password-container">
                            <input type="<?= $label === 'email' ? 'text' : 'password' ?>" class="form-control bg-beige-color password-input" id="<?= $name ?>" name="<?= $name ?>" placeholder="<?= $label ?>" required>
                            <i class="bs bi-eye-slash-fill toggle-eye"  onclick="togglePasswordVisibility('<?= $name ?>')"></i>
                        </div>
                    <?php else: ?>
                        <input type="text" class="form-control form-check-inline bg-beige-color" id="<?= $name ?>" name="<?= $name ?>" placeholder="<?= $label ?>" required>
                    <?php endif; ?>
                </div>
            <?php endforeach; ?>
            <div class="spacer"></div>
